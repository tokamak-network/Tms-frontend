...
import { receiptStatuses } from 'viem/_types/utils/formatters/transactionReceipt';
      ...
      <div className='h-full w-full bg-indigo-950 border border-indigo-950 rounded-md shadow-md'>
        <div className='h-full w-full border border-gray-500 bg-indigo-950 rounded-md mx-auto my-10 p-5'>
          <div className='flex justify-center gap-5 mt-5'>
            <StatCard
              title='Your current multisender approval'
              value={allowance}
            />
            <StatCard
          ...
          <div
            className={`bg-indigo-950 mt-5 ${
              parsedRecipients.length > 5 ? 'overflow-y-auto h-48' : ''
            }`}
          >
            ...
            {parsedRecipients.length > 0 ? (
              parsedRecipients.map((recipient, index) => (
                <div key={index} className='flex justify-between p-3'>
                  <h1 className='text-indigo-800 text-xl'>
                    {recipient.address}
                  </h1>
          ...
          <div className='mt-10'>
            ...
            <div className='flex gap-4 mt-5 ml-4'>
              ...
              />
              <label htmlFor='exact-amount' className='text-white text-xl mt-2'>
